...
[task_local]
#城城领现金
0 0-23/1 * * * https://raw.githubusercontent.com/Alberk/TmallJD/main/jd_city2.js, tag=城城领现金2, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

=================================Loon===================================
[Script]
cron "0 0-23/1 * * *" script-path=https://raw.githubusercontent.com/Alberk/TmallJD/main/jd_city2.js,tag=城城领现金

===================================Surge================================
城城领现金 = type=cron,cronexp="0 0-23/1 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Alberk/TmallJD/main/jd_city2.js

====================================小火箭=============================
城城领现金 = type=cron,script-path=https://raw.githubusercontent.com/Alberk/TmallJD/main/jd_city2.js, cronexpr="0 0-23/1 * * *", timeout=3600, enable=true
 */
const $ = new Env('城城领现金');
...
const author_codes = ['GYzkyuWiRwKkEd7WW5gzmkLsqQPakM-n1e7RNL1n1VA'].sort(() => 0.5 - Math.random())
const self_code = []
let pool = []
!(async () => {
  if (!cookiesArr[0]) {
...
function readShareCode(num=1) {
  return new Promise(async resolve => {
    $.get({url: `https://api.jdsharecode.xyz/api/city123/${num}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
